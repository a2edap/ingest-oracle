...
for i in range(len(ds_cdip["waveTime"])):
    Te[i] = wr.energy_period(
        ds_cdip["waveEnergyDensity"][i].to_pandas()
    ).values.squeeze()
    Tp[i] = wr.peak_period(
        ds_cdip["waveEnergyDensity"][i]
        .to_dataframe("Tp")
        .drop(labels=["waveTime", "metaDeployLatitude", "metaDeployLongitude"], axis=1)
    ).values.squeeze()

ds_cdip["waveTe"] = xr.DataArray(
    Te,
    dims=["waveTime"],
    attrs={
        "units": "s",
